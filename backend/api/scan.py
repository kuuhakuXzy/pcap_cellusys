...
async def reindex_pcaps(request: Request, exclude: Optional[List[str]] = Query(None)):
    ...
    def _background():
        try:
            ...
            result = asyncio.run(scanner_service.scan(settings.PCAP_DIRECTORY, base_url=base_url, exclude=exclude))
...
async def reindex_specific_folder(folder_name: str, request: Request, exclude: Optional[List[str]] = Query(None)):
    ...
    result = await scanner_service.scan(settings.PCAP_DIRECTORY, base_url=base_url, target_folder=folder_name, exclude=exclude)
